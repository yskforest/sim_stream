...
async function changePatientGlbModel(modelId) {
    ...
    if (window.CTModelRegistry && window.Meshes && window.Meshes.patientGroup) {
        ...
        const instance = await CTModelRegistry.spawnModelInstance(modelId, {
            instanceId: "patient_primary",
            attachTo: "couch",
            visible: AppState.patientVisible
        });
        const obj = instance.sceneObject;
...
async function spawnCustomGlbFromInput() {
    const inputEl = document.getElementById("input-add-glb-path");
    if (!inputEl || !inputEl.value.trim()) return;
    const path = inputEl.value.trim();
    const modelId = path.split("/").pop().replace(".glb", "") || "custom_glb";
