(function attachExternalGateway(global) {
    function buildError(requestId, error) {
        ...
        return { requestId: requestId || null, success: false, payload: null, error: error || { code: 'INTERNAL_ERROR', message: 'Unknown error.' } };
    }
    ...
    function execute(command) {
        ...
        if (!result || result.success === false) {
            return buildError(requestId, result && result.error ? result.error : { code: 'INTERNAL_ERROR', message: 'Command execution failed.' });
        }
